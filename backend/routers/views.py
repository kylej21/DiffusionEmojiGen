...
def revert_image(image):
  image = np.round(np.array(image.cpu()*255)).astype(np.uint8)
...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat = 256, embedding_shape=384):
        ...
        self.contextembed1 = nn.Linear(embedding_shape, 2*n_feat)
        self.contextembed2 = nn.Linear(embedding_shape, 1*n_feat)
...
class DDPM(nn.Module):
    ...
    def sample(self, size, device, prompt='happy'):
        ...
        x_i = torch.randn(1, *size).to(device)  # x_T ~ N(0, 1), sample initial noise
        ...
        c_i = torch.tensor(model.encode([prompt]), dtype=torch.float).to(device)
...
ddpm = DDPM(nn_model=ContextUnet(in_channels=3, n_feat=n_feat, embedding_shape=embedding_shape), betas=(1e-4, 0.02), n_T=n_T, device=device, drop_prob=0.1)
ddpm.load_state_dict(torch.load('/Users/kylejohnson/Documents/Hackathons/HackAtUCI/backend/routers/model_199.pth', map_location=torch.device('cpu')))
...
@router.get('/emoji/{prompt}')
def emojiGen(prompt: str):
    ddpm.eval()
    print(f'{prompt}: Generation Started')
    with torch.no_grad():
